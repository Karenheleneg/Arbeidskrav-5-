...
---
Analysere forsøk 
Hypertrofi og gradert proteindose 
- Les Haun et al. og finn hovedhypotesen
- Hvordan motiverer dem hypotesen 
- Hvilke statistiske tester er benyttet for å teste hovedhypotesen?
...
m1 <- hypertrophy %>%
  ...
  mutate(change = VL_T3 - VL_T1,
         ...
         suppl = if_else(GROUP == "MALTO", "CTRL", "PROT")) %>%
 
  lm(change ~ VL_T1 + suppl, data = .)

plot(m1)

#rød strek gir tendensen til hvor rett punktene ligger. Den skal være så rett som mulig. Da det er få deltakere i denne studien er det vanskelig å få den veldig rett 
...
#intercept = hva estimatet er - økning fra T1 til T3 i referansegruppen (GWP) - Den gjennomsnittlige forandringscoren i GWP gruppen. 
# hva er den gjennomsnittlige fremgangen/økningen i GWP gruppen --- 0,84 
#forholdet mellom change score og baseline er negativ - et negativt forhold mellom forandringsscore og baseline 
# --> for hver endring i baseline (T1) verdi på muskeltykkelse så misker change-score med 0,26
# ----- dersom alle gruppene hadde samme utgangspunkt 
# hvis man skal regne ut forandringscoren i e.g. MALTO gruppen, så plusser man sammen intercept estimat med MALTO estimat. 
...
ggplot: ser at mange av prikkene er over linjen, noe som vil tilsi at treningen har hatt en effekt 

- målefeil kan gjøre at informasjonen er noe feil  
- kan også være at utrente bygger muskler raskere enn trente 
...
# fit the model
m3 <- lmer(change ~  VL_T1 + (time * GROUP) + (1|PARTICIPANT), data = dat3)
...
dat4 <- hypertrophy %>%
  ...
  pivot_longer(names_to = "time", values_to = "change", cols = VL_T1:VL_T3) %>%
  print()


m4 <- lmer(log(change) ~ time * GROUP + (1|PARTICIPANT), data = dat4)
